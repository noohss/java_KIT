 ...
 *  -> 부모의 클래스 구성요소를 자식이 사용할 수 있게 된다. 상속되는 요소 필드, 메소드
 *  -> extends를 키워드 사용하여 부모 클래스를 지정
 *  -> private 접근 한정자는 상속 불가
 *  -> protected는 다른 패키지의 자식클래스가 상속받을 수 있도록 한다.
 *  -> 현실 세계에서 반려동물 샵을 운영. 데이터 저장을 위해서 클래스를 정의. 
 *     동물의 종류 개, 고양이, 햄스터, 도마뱀, 토끼, 열대어... -> 하나의 그룹으로 Animal 클래스 정의하고 공통적인 요소를 정의
 *  -> 공통적요소가 아닌것은 구체적인 클래스로 정의(예를들어, Cat, Dog, Rabbit...) 그리고 Animal 클래스를 상속받는다.
 */
