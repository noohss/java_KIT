...
public class C21_StringExAnswer {
	...
	public static void main(String[] args) {
		...
		int alt = email.indexOf('@');
		String account, domain;
		if(alt != -1 && alt == email.lastIndexOf('@')) {
			account = email.substring(0, alt);
			domain = email.substring(alt+1, email.length());
