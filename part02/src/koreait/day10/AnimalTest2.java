...
public class AnimalTest2 {
	public static void main(String[] args) {
		Animal anil = new Frog();		// 업(up)캐스팅 : 자동변환	<-> 다운(down)캐스팅 : 강제변환
		Animal ani2 = new Cat();
		
		Dog puddle = new Dog();
		Crow crow = new Crow();
		// 자식 객체가 생성될때 먼저 부모 클래스 객체를 생성하면서 부모클래스 생성자 실행.
...
//		animal(new Frog());
	...
	public static void animal(Animal animal) {
		// 구현 예시2 : 메소드 재정의. animal 객체가 인스턴스 타입에 따라서 실행되는 act가 다르다.
		animal.act();
		
		animal.print();
	// 인자로 전달받은 animal 객체가 참조하는 것의 진짜 타입(instance 타입) 검사  : instanceof
